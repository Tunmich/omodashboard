...
def estimate_swap_output(chain_name, token_address, amount_in_eth):
    ...
    router = web3.eth.contract(address=Web3.toChecksumAddress(config["router_address"]), abi=router_abi)

    amount_in_wei = web3.toWei(amount_in_eth, "ether")
    path = [Web3.toChecksumAddress(config["router_address"]), Web3.toChecksumAddress(token_address)]
